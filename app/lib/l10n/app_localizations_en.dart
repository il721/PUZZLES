...
class AppLocalizationsEn extends AppLocalizations {
  ...
  String get playgroundRulesPatterns5 =>
      'Twenty-five cardboard tiles are drawn over with red lines. Assemble them into a 5 × 5 square so that the red segments join into one closed line that never crosses itself. The line runs through every tile, so the only thing that can go wrong is a join: a red end meeting a neighbour\'s blank edge, a red end pointing off the square, or the segments closing into several separate loops instead of one. Tiles turn in quarter turns but never flip - the cardboard is one-sided. Tap a tile in the tray to pick it up; tap it again to turn it; then tap an empty place in the square. A tile already laid can be turned in place or sent back to the tray. The number of moves does not matter here - only the finished pattern does. An exhaustive search shows there are exactly 2048 such patterns, counting turns of the whole square.';

  @override
  String get playgroundRulesPatterns4 =>
      'The same red lines, but only sixteen tiles now - exactly the sixteen of the twenty-five that the book\'s own answer uses. Assemble them into a 4 × 4 square so that the red segments join into one closed line that never crosses itself. The line runs through every tile, so the only thing that can go wrong is a join: a red end meeting a neighbour\'s blank edge, a red end pointing off the square, or the segments closing into several separate loops instead of one. Tiles turn in quarter turns but never flip - the cardboard is one-sided. Tap a tile in the tray to pick it up; tap it again to turn it; then tap an empty place in the square. A tile already laid can be turned in place or sent back to the tray. The number of moves does not matter here - only the finished pattern does. An exhaustive search shows there are exactly 512 such patterns, counting turns of the whole square.';

  @override
  String get playgroundRulesSwapBlocks =>
      'A 16 by 8 rectangular box holds twelve identical P-shaped blocks. Each block covers eight cells, so 96 of the 128 cells are filled and 32 stay free — four empty rows across the middle. Two red blocks at the top form a square, two black blocks at the bottom form another. One block moves per move: it slides through the free space horizontally and vertically, any distance, and may turn corners — the whole journey counts as a single move. Blocks are never rotated and never flipped, even when the free space would allow it. The goal is for the red square and the black square to exchange places, with every other block ending up back where it started. Tap a block to select it; dots mark every place it can travel to; press and hold a dot to see a ghost of where the block will land, and release to move it. Illegal destinations are simply never marked. The book prints no answer for this puzzle. The minimum proven by exhaustive search is 32 moves.';
