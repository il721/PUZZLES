...
abstract class AppLocalizations {
  ...
  /// No description provided for @helpRulesRebus.
  ///
  /// In ru, this message translates to:
  ...
  /// No description provided for @helpRulesDigitRebus.
  ///
  /// In ru, this message translates to:
  ...
  /// No description provided for @helpRulesDomino.
  ///
  /// In ru, this message translates to:
  ...
  /// No description provided for @helpRulesLabyrinth.
  ///
  /// In ru, this message translates to:
  ...
  /// No description provided for @helpRulesSquareword.
  ///
  /// In ru, this message translates to:
  ...
  ///
  /// In ru, this message translates to:
  /// **'Этот раздел — не варианты одной головоломки, а семь отдельных маленьких игр: у каждой своё поле и свои правила, которые показывает кнопка ℹ на её экране. Головоломка считается решённой, когда достигнуто целевое положение. Для игр, где важна экономность ходов, приложение также запоминает ваш лучший результат и отмечает его, когда он совпадает с доказанным минимумом. Игры, которые ещё не доступны, показаны серым. Пока открыты только первые шесть — «Восемь фишек», «Кошки и собаки», «Песочные часы», «Всюду по три», «Узоры 5 × 5» и «Узоры 4 × 4»: в «Восьми фишках» фишки 1–8 стоят на концах восьмиконечной звезды, передвигать их можно только по прямым линиям (через свободный центр фишка может проскочить дальше), а цель — расставить их в обратном порядке; доказанный минимум — 28 ходов. В «Кошках и собаках» три кошки и три собаки меняются местами в сквере: за один ход одно животное перебегает на соседнюю свободную площадку, а кошка и собака никогда не должны оказаться рядом; доказанный минимум — 32 хода. В «Песочных часах» пятнадцать фишек нужно перевести из верхнего треугольника в нижний: за ход фишка переходит на соседний свободный кружок или прыгает через соседнюю фишку, как в шашках, причём несколько прыжков подряд одной фишкой считаются одним ходом; доказанный минимум — 26 ходов. В игре «Всюду по три» девять квадратиков 3 × 3 с красными, белыми и чёрными кружками нужно сложить в квадрат 9 × 9 так, чтобы на каждой горизонтали, вертикали и обеих диагоналях было ровно по одному кружку каждого цвета; квадратики поворачиваются, но не переворачиваются, а число ходов здесь не важно. В «Узорах 5 × 5» двадцать пять квадратиков с красными линиями нужно сложить в квадрат 5 × 5 так, чтобы все отрезки сомкнулись в одну замкнутую линию без самопересечений; квадратики поворачиваются, но не переворачиваются, а число ходов и здесь не важно. В «Узорах 4 × 4» та же линия собирается всего из шестнадцати квадратиков — ровно тех, которыми сложен ответ в книге. Теперь открыты все семь: в «Поменяйте местами» двенадцать одинаковых P-образных блоков заполняют ящик, за один ход один блок скользит по свободному пространству и может поворачивать за углы, но никогда не поворачивается; цель — поменять местами красный и чёрный квадраты, вернув остальные блоки на исходные места; доказанный минимум — 32 хода.'**
  ...
  /// Rules sheet body for the cats_dogs game.
  ///
  /// In ru, this message translates to:
  /// **'Три кошки (C) сидят на левых площадках сквера, три собаки (D) — на правых. За один ход одно животное перебегает по аллее на соседнюю свободную площадку. Кошка и собака никогда не должны оказаться на соседних площадках — такие ходы просто не предлагаются. Цель — поменять кошек и собак местами. Доказанный минимум — 32 хода: столько же обещали кошки в книге, и они оказались правы.'**
  String get playgroundRulesCatsDogs;

  /// Rules sheet body for the hourglass game.
  ///
  /// In ru, this message translates to:
  /// **'Пятнадцать фишек заполняют верхний треугольник песочных часов; их нужно все перевести в нижний. За один ход фишка либо переходит по линии на соседний свободный кружок, либо перепрыгивает, как в шашках, через соседнюю фишку на свободный кружок сразу за ней. Несколько прыжков подряд одной фишкой считаются одним ходом, и остановиться можно после любого прыжка; смешивать переход и прыжки в одном ходе нельзя. Двигаться можно только по нарисованным линиям. В книге ответа нет — доказанный перебором минимум составляет 26 ходов.'**
  String get playgroundRulesHourglass;

  /// Rules sheet body for the three_each game.
  ///
  /// In ru, this message translates to:
  /// **'Девять картонных квадратиков 3 × 3 расписаны красными, белыми и чёрными кружками. Сложите из них квадрат 9 × 9 так, чтобы на каждой из девяти горизонталей, на каждой из девяти вертикалей и на обеих больших диагоналях оказалось ровно по одному кружку каждого цвета. Пустые клетки не в счёт: кружков всего 27, то есть ровно по три на линию. Квадратики можно поворачивать на четверть оборота, но не переворачивать — картон односторонний. Нажмите на квадратик в наборе, чтобы выбрать его; нажмите ещё раз — он повернётся; затем нажмите на свободное место в квадрате. Уже выложенный квадратик можно повернуть на месте или вернуть в набор. Число ходов здесь не важно — важен только собранный квадрат. Перебор показывает, что таких квадратов ровно шестнадцать, считая повороты всего квадрата.'**
  String get playgroundRulesThreeEach;

  /// Rules sheet body for the patterns5 game.
  ///
  /// In ru, this message translates to:
  /// **'Двадцать пять картонных квадратиков расчерчены красными линиями. Сложите из них квадрат 5 × 5 так, чтобы красные отрезки сомкнулись в одну замкнутую линию, которая нигде не пересекает сама себя. Линия проходит по всем квадратикам, поэтому ошибиться можно только на стыках: красный конец упирается в пустой край соседа, торчит за границу квадрата или отрезки замыкаются в несколько отдельных петель вместо одной. Квадратики можно поворачивать на четверть оборота, но не переворачивать — картон односторонний. Нажмите на квадратик в наборе, чтобы выбрать его; нажмите ещё раз — он повернётся; затем нажмите на свободное место в квадрате. Уже выложенный квадратик можно повернуть на месте или вернуть в набор. Число ходов здесь не важно — важен только сложенный узор. Перебор показывает, что таких узоров ровно 2048, считая повороты всего квадрата.'**
  String get playgroundRulesPatterns5;

  /// Rules sheet body for the patterns4 game.
  ///
  /// In ru, this message translates to:
  /// **'Те же красные линии, но квадратиков теперь шестнадцать — ровно те шестнадцать из двадцати пяти, которыми сложен ответ в книге. Сложите из них квадрат 4 × 4 так, чтобы красные отрезки сомкнулись в одну замкнутую линию, которая нигде не пересекает сама себя. Линия проходит по всем квадратикам, поэтому ошибиться можно только на стыках: красный конец упирается в пустой край соседа, торчит за границу квадрата или отрезки замыкаются в несколько отдельных петель вместо одной. Квадратики можно поворачивать на четверть оборота, но не переворачивать — картон односторонний. Нажмите на квадратик в наборе, чтобы выбрать его; нажмите ещё раз — он повернётся; затем нажмите на свободное место в квадрате. Уже выложенный квадратик можно повернуть на месте или вернуть в набор. Число ходов здесь не важно — важен только сложенный узор. Перебор показывает, что таких узоров ровно 512, считая повороты всего квадрата.'**
  String get playgroundRulesPatterns4;

  /// Rules sheet body for the swap_blocks game.
  ///
  /// In ru, this message translates to:
  /// **'Прямоугольный ящик 16 × 8 заполнен двенадцатью одинаковыми P-образными блоками: каждый занимает восемь клеток, поэтому заняты 96 из 128 клеток, а 32 остаются свободными — четыре пустые строки посередине. Два красных блока сверху образуют квадрат, два чёрных внизу — ещё один. За один ход двигается один блок: он скользит по свободному пространству горизонтально и вертикально на любое расстояние и может поворачивать за углы — всё это путешествие считается одним ходом. Блоки никогда не поворачиваются и не переворачиваются, даже если свободное место это допускает. Цель — красный квадрат и чёрный квадрат должны поменяться местами, а все остальные блоки вернуться на свои места. Нажмите на блок, чтобы выбрать его; точки отмечают каждое место, куда он может переехать; нажмите и удерживайте точку, чтобы увидеть призрак будущего места блока, и отпустите, чтобы его передвинуть. Недопустимые места просто не отмечаются. В книге ответа нет — доказанный перебором минимум составляет 32 хода.'**
