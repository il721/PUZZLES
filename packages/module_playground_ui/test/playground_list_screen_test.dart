...
void main() {
  ...
  testWidgets(
      'renders seven rows; exactly four are tappable; three show comingSoon',
      (tester) async {
    ...
    expect(tiles.where((t) => t.enabled).length, 4);
    expect(tiles.where((t) => t.onTap != null).length, 4);
  ...
  testWidgets('tapping a disabled row does not navigate', (tester) async {
    ...
    // 'patterns5' is a disabled game in the fixed registry order.
    await tester.tap(find.byKey(const ValueKey('pg-row-patterns5')));
    await _settle(tester);
