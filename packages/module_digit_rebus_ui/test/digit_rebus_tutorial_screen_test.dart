...
/// A minimal [DigitRebusL10n] for widget tests, independent of any real
/// localization setup.
class _FakeDigitRebusL10n implements DigitRebusL10n {
  const _FakeDigitRebusL10n();

  @override
  String get puzzleListTitle => 'Digit rebuses';
  @override
...
List<DigitRebusPuzzle> _loadPuzzles() {
  // Tests run from the package directory (`flutter test` in
  // packages/module_digit_rebus_ui); module_digit_rebus is a sibling.
  final file = File('../module_digit_rebus/assets/puzzles/module02.json');
  return DigitRebusPuzzle.listFromJsonString(file.readAsStringSync());
}

/// Wraps [DigitRebusTutorialScreen] behind a "home" screen with a button
/// that pushes it, so tests can verify a Done/Skip/close action actually
/// pops back.
Widget _wrapScreen({
  required SaveService saveService,
  required List<DigitRebusPuzzle> puzzles,
}) {
  return ProviderScope(
    overrides: [
      digitRebusPuzzlesProvider.overrideWith((ref) async => puzzles),
      digitRebusSaveServiceProvider.overrideWithValue(saveService),
      digitRebusL10nProvider.overrideWithValue(const _FakeDigitRebusL10n()),
    ],
    child: MaterialApp(
      home: Builder(
...
void main() {
  ...
  setUp(() {
    ...
    tempDir = Directory.systemTemp.createTempSync('module_digit_rebus_ui_tutorial_test_');
  ...
  testWidgets('tapping Next advances the step, and reveals row 2 by step 4', (tester) async {
    await openTutorial(tester);
    ...
    await tapNext(tester); // step 3 — highlights row 2, no reveal yet
    await tapNext(tester); // step 4 — reveals row 2 (1, 2, 3, 9)

    expect(find.text('step 4'), findsOneWidget);
    const revealed = CellRef(1, 0);
    expect(find.descendant(of: _cellContainer(revealed), matching: find.text('1')), findsOneWidget);
    expect(
        find.descendant(of: _cellContainer(const CellRef(1, 3)), matching: find.text('9')),
        findsOneWidget);
  });
  ...
  testWidgets(
      ...
      'tutorial-completion flag, preserving pre-existing save data', (tester) async {
    ...
    // tutorial.
    await tester.runAsync(() => saveService.save('module_digit_rebus', <String, dynamic>{
    ...
    expect(find.text('step ${tutorialSteps.length}'), findsOneWidget);
    ...
    for (var i = 0; i < 50 && saved == null; i++) {
      ...
      await tester.pump();
      final data = (await tester.runAsync(() => saveService.load('module_digit_rebus')))!;
