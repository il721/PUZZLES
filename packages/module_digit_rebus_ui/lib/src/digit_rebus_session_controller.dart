...
/// Sentinel used by [DigitRebusSessionState.copyWith] to distinguish "leave
...
class DigitRebusSessionState {
  /// The player's current grid (mutable in place; a new
  /// [DigitRebusSessionState] is published after every mutation so
  ...
  factory DigitRebusSessionState.initial(DigitRebusPuzzle puzzle) =>
      DigitRebusSessionState(grid: PlayerGrid.fromPuzzle(puzzle));
  ...
  DigitRebusSessionState copyWith({
    PlayerGrid? grid,
...
class DigitRebusSessionNotifier extends Notifier<DigitRebusSessionState> {
  /// Creates a session notifier for [puzzleId].
  DigitRebusSessionNotifier(this.puzzleId);

  /// The id of the puzzle this session plays.
  final String puzzleId;

  late final DigitRebusPuzzle _puzzle;
  ...
  @override
  DigitRebusSessionState build() {
    final puzzles = ref.watch(digitRebusPuzzlesProvider).requireValue;
    _puzzle = puzzles.firstWhere((p) => p.id == puzzleId);
    _saveService = ref.read(digitRebusSaveServiceProvider);
    ...
    return DigitRebusSessionState.initial(_puzzle);
  }
  ...
  Future<void> _loadPersisted() async {
    final namespaceData = await _saveService.load(digitRebusSaveNamespace);
  ...
  /// selection to the next empty cell in reading order. A no-op if nothing
  /// is selected or the grid is locked for review. Glyph-set membership is
  /// deliberately NOT enforced here — the verifier owns it (defense in
  /// depth); input UIs restrict the choices they offer instead.
  ...
  /// no-op while the grid is locked for review.
  ...
  /// steps back to the previous cell and clears that one instead. A no-op
  /// if nothing is selected or the grid is locked for review.
  void backspace() {
    final sel = state.selected;
    if (sel == null || state.reviewMode) return;
  ...
  /// Clears every cell back to blank. Only valid for an unsolved puzzle
  /// (the caller is expected to confirm with the player first).
  void reset() {
  ...
  Future<void> _persistNow() async {
    ...
    // top-level keys — e.g. a future tutorial-completion flag — survive
    // this write instead of being dropped.
    ...
    await _saveService.save(digitRebusSaveNamespace, merged);
  }
}
